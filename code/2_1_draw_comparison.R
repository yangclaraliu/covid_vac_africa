...
merge_novac <- function(tmp, ext = F){
  require(magrittr)
  if(ext == F){
...
# res <- merge_novac(res)
# res_ext <- merge_novac(res_ext, ext = T)
...
res$pfizer %>%
  ...
  scale_x_continuous(breaks = seq(ymd("2021-01-01"), ymd("2021-12-01"), "month"),
                     labels = paste0(month(seq(ymd("2021-01-01"), ymd("2021-12-01"), "month"), label = T), "\n2021"),
                     limits = c(ymd("2020-12-15"), ymd("2021-12-15"))) +
  custom_theme

# ggsave("figs/epi_outcomes_vertical_v4.png", width = 15, height = 8)
ggsave("figs/fig2_pf.png", width = 15, height = 8)
